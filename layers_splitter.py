...
ARC_START = ''': Arc Start[{job}];'''
ARC_END = ''': Arc End[{job}];'''

WEAVE_SINE = ''':  Weave Sine[{job}];'''
WEAVE_END = ''': Weave End[{job}];'''
  ...
  for line in f:
    ...
    if flag:
      if line[:-1] == '};':
        ...
        coords = s.split('\n')[5].split()
        w, p, r = float(coords[2]), float(coords[6]), float(coords[10])
        layers.add(z)
        print([x, y, z, w, p, r])
        positions.append([x, y, z, w, p, r])
        s = ''
      s = '{}{}'.format(s, line)
    else:
      command = line.split()
      if len(command) > 2:
        ...
        elif 'Arc' in command[1] and 'Start' in command[2]:
          commands.append('Arc Start')
        elif 'Arc' in command[1] and 'End' in command[2]:
          commands.append('Arc End')
##        elif 'Weave' in command[1] and 'Sine' in command[2]:
##          commands.append('Weave Sine')
##        elif 'Weave' in command[1] and 'End' in command[2]:
##          commands.append('Weave End')
...
def make_layers(commands, positions, settings):
  ...
  for i in range(len(commands)):
    if commands[i] == 'Arc Start':
      weld_speed = True
      cs.append(ARC_START.format(job=1))
      start_ind = len(cs) - 1
      start_point = positions[int(commands[i - 1]) - 1]
      cs.append(WEAVE_SINE.format(job=1))

    elif commands[i] == 'Arc End':
      weld_speed = False
      if start_ind != None:
        """Проверка наличия замкнутого контура."""
        if start_point == positions[int(commands[i - 1]) - 1]:
          job = settings[len(layers)][1]
          job2 = settings[len(layers)][2]
        else:
          job = settings[len(layers)][0]
          job2 = settings[len(layers)][2]
        cs[start_ind] = ARC_START.format(job=job)
        cs.append(ARC_END.format(job=job))
        cs[start_ind+1] = WEAVE_SINE.format(job=job2)
        cs.append(WEAVE_END.format(job=job2))
      ...
      if positions[index][2] !=  z or z is None:
        ...
        line_number = 1
        position_index = 1
        layers.append([cs.copy(), ps.copy(), zs.copy()])
        cs, ps, zs = [], [], []
        start_ind = None
      if weld_speed:
...
def build_ls(otput_folder, i, layer, last):
  otput_file = '{}/{}_{}_{}.ls'.format(otput_folder, otput_folder, str(layer[2][0]).replace('.', ''), i+1)
  f = open(otput_file, 'w')
  f.write('{}\n'.format(HEAD.format(program_name='{}_{}_{}'.format(otput_folder, str(layer[2][0]).replace('.', ''), i+1),
...
def build_ls_files(otput_folder, commands, positions, settings):
  ...
  for i, layer in enumerate(layers):
    ...
    f.write('{}\n'.format(NEXT_PROGRAM.format(program_name='{}_{}_{}'.format(otput_folder, str(layer[2][0]).replace('.', ''), i+1))))
  f.write('{}\n'.format(POS_START))
  f.write('{}\n'.format(POS_END))
  f.close()
...
if __name__ == '__main__':
  ...
  def button_build():
    global status, commands, positions, layers_settings_names, layers_settings_common, layers_settings_contour, layers_settings_weave, out_folder
  ...
  try:  
    ...
    layers_settings_weave = [tk.IntVar() for i in range(SETTINGS_COUNT)]
    ...
    tk.Label(text="Weave").grid(row=3, column=3)
    for i in range(SETTINGS_COUNT):
      ...
      tk.Entry(master, textvariable=layers_settings_weave[i]).grid(row=i + 4, column=3) 
